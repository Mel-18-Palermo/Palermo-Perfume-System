...
```mermaid
flowchart LR
    Browser["Customer browser"] --> App["Next.js application"]
...
The first adapter will use the OpenAI Responses API from server-only TypeScript over HTTPS. The
model and prompt are configuration choices to approve during implementation; they are not fixed in
this specification.
