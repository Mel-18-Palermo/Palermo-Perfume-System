...
The assessment covers the planned Next.js website, Supabase Postgres database, administrator access,
payment sandbox, AI-assisted support, application logs, and backups. It covers customers, visitors
who submit data, and administrators whose actions are logged.

Selecting Vercel and Supabase does not complete the required privacy, hosting-location, contractual,
or security review. The payment provider, production AI configuration, analytics tool, marketing
platform, and backup process also require approval.
...
flowchart LR
    ...
    App -->|"authorised records"| DB[("Supabase Postgres")]
...
retention rule, or hosting control has not been verified.
