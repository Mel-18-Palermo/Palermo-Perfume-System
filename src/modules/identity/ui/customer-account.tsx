  ...
  return <div className="grid gap-4 sm:grid-cols-2">
    {fields.map(field => <label key={field.key} className={field.key === "line1" || field.key === "line2" ? "sm:col-span-2" : ""}>
      ...
      <input id={`${prefix}-${field.key}`} name={field.key} required={field.key !== "line2"} autoComplete={field.autoComplete} disabled={disabled}
        ...
        className="min-h-[44px] w-full rounded-md border border-border-strong bg-surface px-3 py-2 text-sm text-text outline-none focus:border-info disabled:opacity-60" />
    </label>)}
  </div>;
}
...
export function CustomerAccount() {
  ...
  async function mutate(label: string, action: (current: CustomerProfile) => ReturnType<typeof api.profile.update>): Promise<void> {
    if (!profile || busy) return;
    setBusy(label); setMessage(null);
    try {
      const result = await action(profile);
      if (result.ok) { sync(result.data); setMessage({ variant: "success", text: "Your account details have been saved." }); }
  ...
  function saveProfile(event: React.FormEvent<HTMLFormElement>) {
    ...
    void mutate("profile", current => api.profile.update({ expectedRevision: current.revision, name, preferences: { favouriteNoteIds: noteIds, preferredIntensityId: intensityId || null, sensitivityAvoidance: avoidance.trim() || null } }));
  }
  function saveDelivery(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    void mutate("delivery", current => api.profile.setDeliveryAddress({ expectedRevision: current.revision, address: addressInput(delivery) }));
  }
  function saveBilling(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    void mutate("billing", current => api.profile.setBillingAddress({ expectedRevision: current.revision, billing: sameAsDelivery ? { kind: "USE_DELIVERY" } : { kind: "SEPARATE", address: addressInput(billing) } }));
  }
  function generateIdentity() { void mutate("identity", current => api.profile.generateIdentity({ expectedRevision: current.revision })); }
      ...
      {!loading && profile && <>
        <Card><CardHeader><CardTitle>Profile and preferences</CardTitle></CardHeader><CardContent>
          <form className="space-y-5" onSubmit={saveProfile}>
            <label><span className="mb-1 block text-label text-text">Name</span><input required autoComplete="name" disabled={disabled} value={name} onChange={event => setName(event.target.value)} className="min-h-[44px] w-full rounded-md border border-border-strong bg-surface px-3 py-2 text-sm text-text outline-none focus:border-info disabled:opacity-60" /></label>
            ...
            <label><span className="mb-1 block text-label text-text">Preferred intensity</span><select disabled={disabled || !filters} value={intensityId} onChange={event => setIntensityId(event.target.value)} className="min-h-[44px] w-full rounded-md border border-border-strong bg-surface px-3 py-2 text-sm text-text"><option value="">No preference</option>{filters?.intensity.map(option => <option key={option.id} value={option.id}>{option.label}</option>)}</select></label>
            <label><span className="mb-1 block text-label text-text">Fragrance avoidance (optional)</span><textarea disabled={disabled} maxLength={500} value={avoidance} onChange={event => setAvoidance(event.target.value)} className="min-h-24 w-full rounded-md border border-border-strong bg-surface px-3 py-2 text-sm text-text outline-none focus:border-info disabled:opacity-60" /><span className="mt-1 block text-xs text-text-muted">Share scent preferences only; do not include medical information.</span></label>
