...
type CopyFeedback = { message: string; ok: boolean };
...
export function ReferralPanel() {
  ...
  const handleCopy = (target: CopyTarget, value: string) => {
    ...
    if (typeof navigator === "undefined" || navigator.clipboard === undefined) {
      setCopyFeedback({
        message: `${label} could not be copied automatically. Select the text to copy it.`,
        ok: false,
      });
      return;
    }
    ...
    navigator.clipboard
      .writeText(value)
      .then(() => {
        setCopyFeedback({ message: `${label} copied to your clipboard.`, ok: true });
      })
      .catch(() => {
        setCopyFeedback({
          message: `${label} could not be copied. Select the text to copy it.`,
          ok: false,
        });
      });
  };
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <p
                role="status"
                aria-live="polite"
                className={`min-h-4 text-xs ${copyFeedback?.ok === false ? "text-danger" : "text-success"}`}
              >
