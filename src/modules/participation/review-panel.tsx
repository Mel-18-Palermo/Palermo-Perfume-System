...
export function ReviewPanel({ perfumeId, perfumeName }: ReviewPanelProps) {
  ...
  // Stable across a retry of the same content; only regenerated when the rating or
  // body actually changes, so a resubmit after a failure cannot create a duplicate
  // review (frontend-contracts.md §18).
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    submitPerfumeReview(
      { perfumeId, rating, body: trimmedBody, idempotencyKey },
      readParticipationScenario()
    )
      .then((result) => {
    ...
    <section aria-labelledby="customer-reviews-heading" className="space-y-4">
      ...
      {status === "ready" && data !== null && (
        <div className="space-y-6">
          {/* Moderation / pending presentation for the customer's own review. Right
              after submission this leads with the server's own confirmation copy;
              on a later visit it falls back to a generic status sentence. */}
          {data.ownPendingReview !== null && (
            <Alert variant="warning" title="Your review is awaiting moderation">
              {submitMessage ??
                `Submitted ${formatDate(data.ownPendingReview.submittedAt)}. It will appear in the public list once the Palermo team has approved it.`}
