...
export class ParticipationService {
  ...
  async rewardQualifyingReferral(orderId: string, points: number): Promise<ApiResult<{ awarded: boolean }>> {
    ...
    const result = await this.db.$transaction(async tx => {
      ...
      await tx.loyaltyAccount.createMany({
        data: [{ customerId: referral.referrerCustomerId }],
        skipDuplicates: true,
      });
      const account = await tx.loyaltyAccount.findUniqueOrThrow({ where: { customerId: referral.referrerCustomerId } });
      const inserted = await tx.loyaltyLedgerEntry.createMany({
        data: [{ accountId: account.id, type: "REFERRAL_REWARD", points, identity: `referral:${referral.id}`, orderId: null, referralId: referral.id }],
        skipDuplicates: true,
      });
      if (inserted.count === 0) return "DONE" as const;
  ...
  private async credit(customerId: string, points: number, identity: string, type: "ORDER_REWARD" | "REFERRAL_REWARD", orderId?: string, referralId?: string): Promise<ApiResult<{ awarded: boolean }>> {
    const result = await this.db.$transaction(async tx => {
      ...
      const account = await tx.loyaltyAccount.findUniqueOrThrow({ where: { customerId } });
      const inserted = await tx.loyaltyLedgerEntry.createMany({
        data: [{ accountId: account.id, type, points, identity, orderId: orderId ?? null, referralId: referralId ?? null }],
        skipDuplicates: true,
      });
