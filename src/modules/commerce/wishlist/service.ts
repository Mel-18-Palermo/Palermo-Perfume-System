...
const include = { perfume: { include: { primaryFamily: true, intensity: true, images: { orderBy: { sortOrder: "asc" as const } }, variants: { select: { availability: true, priceMinor: true, currency: true } } } } } as const;
...
export class WishlistService {
  ...
  private dto(items: readonly Loaded[]): WishlistDto { return { items: items.map(item => ({ perfumeId: item.perfumeId, perfume: item.perfume.status === "ACTIVE" ? summary(item) : null, available: item.perfume.status === "ACTIVE" && item.perfume.variants.some(variant => variant.availability === "AVAILABLE") })) }; }
