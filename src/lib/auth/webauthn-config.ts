...
export function webAuthnConfig(request: Request): WebAuthnConfig {
  ...
  const preview = process.env["VERCEL_ENV"] === "preview" && url.hostname.endsWith(".vercel.app");
  if ((process.env["NODE_ENV"] !== "production" && local) || preview) return { rpID: url.hostname, origin: url.origin };
