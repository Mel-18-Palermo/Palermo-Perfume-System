...
const status = (result: { readonly ok: boolean; readonly error?: { readonly code: string } }): number => result.ok ? 200 : ({ UNAUTHENTICATED: 401, FORBIDDEN: 403, NOT_FOUND: 404, CONFLICT: 409, VALIDATION_ERROR: 400, TEMPORARILY_UNAVAILABLE: 503, INTEGRATION_ERROR: 502 }[result.error?.code ?? ""] ?? 500);
