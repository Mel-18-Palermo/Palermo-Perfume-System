...
export function checkoutCases(db: PrismaClient): void {
  describe("authoritative checkout boundary", () => {
    ...
    it("requires a customer and exposes active delivery methods", async () => { expect(status(await service.getDeliveryMethods())).toContain("Demo delivery"); expect(status(await service.submit("bad", {} as never))).toBe("VALIDATION_ERROR"); });
    it("revalidates, reserves and replays an idempotent checkout request", async () => {
      ...
      await db.payment.deleteMany({ where: { orderId: first.data.orderId } }); await db.inventoryReservation.deleteMany({ where: { orderId: first.data.orderId } }); await db.orderItem.deleteMany({ where: { orderId: first.data.orderId } }); await db.order.delete({ where: { id: first.data.orderId } }); await db.cart.delete({ where: { id: cart.id } }); await db.address.delete({ where: { id: address.id } });
    });
